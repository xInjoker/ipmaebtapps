...
export default function FlashReportListPage() {
  ...
  const flashReports = useMemo(() => {
    const filteredByType = reports.filter(r => r.jobType === 'Flash Report');
    if (userRole?.id === 'inspector' || userRole?.id === 'employee') {
  ...
  const dashboardStats = useMemo(() => {
    const totalItems = flashReports.reduce((acc, report) => acc + report.qtyJoint, 0); // using qtyJoint for quantity
    const statusCounts = flashReports.reduce((acc, report) => {
      acc[report.status] = (acc[report.status] || 0) + 1;
      return acc;
    }, {} as Record<ReportStatus, number>);

    return {
      totalReports: flashReports.length,
      totalItems,
  ...
  return (
    <>
      <div className="space-y-6">
        <HeaderCard
            title="Flash Reports (QMS)"
            description="View and manage all flash reports."
        >
            <Button asChild>
              <Link href="/reports/flash/new">
